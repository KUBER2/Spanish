...
$wordsQuery = $db->query('SELECT dictionary.Word, dictionary.Translation, category.Category FROM dictionary INNER JOIN category ON dictionary.Category=category.CategoryID');
$words =  $wordsQuery->fetchAll();
//print_r($slowa);
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-dark">
	...
	<div class="container">
		<div class = "m-4 p-5">
			Wyświetlaj po <button type="button" class="btn btn-info btn-large" id="wordsBy10">10</button> <button type="button" id="wordsBy10" class="btn btn-info btn-sm">20</button> słów.
		</div>
		<p>
				...
				<tbody id="wordsTable">
				  <?php
					foreach ($words as $Word){
						echo "<tr><td>{$Word['Word']}</td><td>{$Word['Translation']}</td><td>{$Word['Category']}</td></tr>";
					}				  
				  ?>
				</tbody>
			</table>
